...
class FindAssignableAllocationService
{
     ...
     *
     * @param \river\Services\Allocations\AssignmentService $service
     */
     ...
     *
     * @return \river\Models\Allocation
     *
    ...
    {
        if (!config('river.client_features.allocations.enabled')) {
        ...
        // server.
        /** @var \river\Models\Allocation|null $allocation */
    ...
    {
        $start = config('river.client_features.allocations.range_start', null);
        $end = config('river.client_features.allocations.range_end', null);
        ...
        /** @var \river\Models\Allocation $allocation */
        $allocation = $server->node->allocations()
            ->where('ip', $server->allocation->ip)
            ->where('port', $port)
            ->firstOrFail();
