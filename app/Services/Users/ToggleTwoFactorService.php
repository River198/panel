...
use river\Repositories\Eloquent\RecoveryTokenRepository;
use river\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid;
...
class ToggleTwoFactorService
{
    ...
    /**
     * @var \river\Repositories\Eloquent\RecoveryTokenRepository
     */
     ...
     * @throws \river\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid
     */
    public function handle(User $user, string $token, bool $toggleState = null): array
    {
        $secret = $this->encrypter->decrypt($user->totp_secret);

        $isValidToken = $this->google2FA->verifyKey($secret, $token, config()->get('river.auth.2fa.window'));
