...
namespace river\Services\Helpers;
...
use river\Exceptions\Service\Helper\CdnVersionFetchingException;

class SoftwareVersionService
{
    public const VERSION_CACHE_KEY = 'river:versioning_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://river.io/discord';
    }
    ...
    {
        return Arr::get(self::$result, 'donations') ?? 'https://paypal.me/riverSoftware';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config()->get('river.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config()->get('river.cdn.url'));
