...
namespace river\Console\Commands\Environment;

use PDOException;
use Illuminate\Console\Command;
use Illuminate\Contracts\Console\Kernel;
use Illuminate\Database\DatabaseManager;
use river\Traits\Commands\EnvironmentWriterTrait;
...
class DatabaseSettingsCommand extends Command
{
     ...
     * @return int
     *
     * @throws \river\Exceptions\riverException
     */
    public function handle()
    {
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            'Database Username',
            config('database.connections.mysql.username', 'river')
        );
        ...
        try {
            ...
            if ($this->confirm('Go back and try again?')) {
                $this->database->disconnect('_river_command_test');
    ...
    {
        config()->set('database.connections._river_command_test', [
            'driver' => 'mysql',
        ...
        $this->database->connection('_river_command_test')->getPdo();
    }
}
