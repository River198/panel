...
namespace river\Http\Controllers\Auth;
...
use river\Http\Requests\Auth\LoginCheckpointRequest;
...
class LoginCheckpointController extends AbstractLoginController
{
    ...
    public function __invoke(LoginCheckpointRequest $request): JsonResponse
    {
        ...
        if (!is_null($recoveryToken = $request->input('recovery_token'))) {
            ...
            if ($this->google2FA->verifyKey($decrypted, (string) $request->input('authentication_code') ?? '', config('river.auth.2fa.window'))) {
