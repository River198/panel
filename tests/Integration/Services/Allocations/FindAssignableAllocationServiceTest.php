...
namespace river\Tests\Integration\Services\Allocations;

use Exception;
use InvalidArgumentException;
use river\Models\Allocation;
use river\Tests\Integration\IntegrationTestCase;
use river\Services\Allocations\FindAssignableAllocationService;
use river\Exceptions\Service\Allocation\AutoAllocationNotEnabledException;
use river\Exceptions\Service\Allocation\NoAutoAllocationSpaceAvailableException;
...
class FindAssignableAllocationServiceTest extends IntegrationTestCase
{
    ...
    public function setUp(): void
    {
        ...
        config()->set('river.client_features.allocations.range_start', 0);
        config()->set('river.client_features.allocations.range_end', 0);
    }
    ...
    public function testNewAllocationIsCreatedIfOneIsNotFound()
    {
        ...
        config()->set('river.client_features.allocations.range_end', 5005);
    ...
    public function testOnlyPortNotInUseIsCreated()
    {
        ...
        config()->set('river.client_features.allocations.range_start', 5000);
        config()->set('river.client_features.allocations.range_end', 5001);
    ...
    {
        $server = $this->createServerModel();
        config()->set('river.client_features.allocations.range_start', 'hodor');
        config()->set('river.client_features.allocations.range_end', 10);
        ...
        config()->set('river.client_features.allocations.range_start', 10);
        config()->set('river.client_features.allocations.range_end', 'hodor');
    ...
    /**
     * @return \river\Services\Allocations\FindAssignableAllocationService
     */
