...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(river.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(river.server.egg_id) && river.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(river.server.egg_id)) {
                if ($('#pDockerImage').val() != river.server.image) {
                    $('#pDockerImageCustom').val(river.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(river.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(river.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(river.nests, $(this).val() + '.eggs.' + river.server.egg_id))) {
                $('#pEggId').val(river.server.egg_id);
            }
